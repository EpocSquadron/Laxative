...
# Check if ee-master-config exists for this env, then pull form that
	HOST=`grep -Pio "(?<=hostname\'\]\ \=\ \')([a-z\@\#%^&*()_+.:]+)(?=\')" "$EEMASTERCONF_FILE"`
	PORT=`grep -Pio "(?<=port\'\]\ \=\ \')([a-z\@\#%^&*()_+.:]+)(?=\')" "$EEMASTERCONF_FILE"`
	USER=`grep -Pio "(?<=username\'\]\ \=\ \')([a-z\@\#%^&*()_+.:]+)(?=\')" "$EEMASTERCONF_FILE"`
	PASS=`grep -Pio "(?<=password\'\]\ \=\ \')([a-z\@\#%^&*()_+.:]+)(?=\')" "$EEMASTERCONF_FILE"`
	DB=`grep -Pio "(?<=database\'\]\ \=\ \')([a-z\@\#%^&*()_+.:]+)(?=\')" "$EEMASTERCONF_FILE"`
